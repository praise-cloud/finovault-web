  ...
  return (
    <section aria-label="Financial Mirror" className="bg-[#0a0e17] py-24 md:py-32">
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-12 px-6 lg:grid-cols-2 md:px-10">
        {/* Text */}
        <div>
          <p className="text-[0.75rem] font-bold uppercase tracking-[0.1em] text-[var(--fv-hp-accent)]">
            {t('hp.mirror.eyebrow')}
          </p>
          <h2
          ...
          <p className="mt-6 max-w-[55ch] text-[var(--fv-hp-body-lg)] font-medium leading-relaxed text-[var(--fv-hp-text-body)]">
        ...
        >
          {categories.map(({ nameKey, pctKey, width, color }) => (
            <div key={nameKey} className="mb-6 last:mb-0">
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-medium text-[var(--fv-hp-text-body)]">{t(nameKey)}</span>
                <span className="hp-display text-sm font-bold text-white">{t(pctKey)}</span>
              </div>
          ...
          <p className="mt-8 text-[0.75rem] font-bold uppercase tracking-[0.1em] text-[var(--fv-hp-text-muted)]">
            {t('hp.illustrative')}
          </p>
        </div>
      </div>
