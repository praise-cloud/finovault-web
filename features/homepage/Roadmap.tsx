...
const phases = [
  { key: 'phase1', statusClass: 'text-[#6366f1] border-[#6366f1]/40' },
        ...
        <div className="relative mt-16 grid grid-cols-1 gap-8 md:grid-cols-3">
          ...
          {phases.map(({ key, statusClass }) => (
            <div key={key} className="relative border border-white/12 p-8">
              <p className="text-sm text-white/50">{t(`hp.roadmap.${key}.phase`)}</p>
              <h3 className="hp-display mt-2 text-[var(--fv-hp-display-md)] text-white">
                {t(`hp.roadmap.${key}.title`)}
              </h3>
              <span
                className={`mt-3 inline-block rounded-none border px-2 py-0.5 text-[0.65rem] uppercase tracking-[0.1em] ${statusClass}`}
              >
